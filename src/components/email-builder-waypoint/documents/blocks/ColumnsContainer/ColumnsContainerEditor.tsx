...
export default function ColumnsContainerEditor({ style, props }: ColumnsContainerProps) {
  ...
  const updateColumn = (columnIndex: 0 | 1 | 2, { block, blockId, childrenIds }: EditorChildrenChange) => {
    const nColumns = [...(columnsValue as { childrenIds: string[] }[])];
  ...
  return (
    <BaseColumnsContainer
      ...
      style={style}
      columns={[
        <EditorChildrenIds childrenIds={(columns as any)?.[0]?.childrenIds} onChange={(change) => updateColumn(0, change)} />,
        <EditorChildrenIds childrenIds={(columns as any)?.[1]?.childrenIds} onChange={(change) => updateColumn(1, change)} />,
        <EditorChildrenIds childrenIds={(columns as any)?.[2]?.childrenIds} onChange={(change) => updateColumn(2, change)} />,
      ]}
    />
  );
}
