...
interface WaypointEditorClientProps {
    ...
    savedBlocks: any[];
    clientSlug?: string;
    saveEndpoint?: string; // Optional custom endpoint override
}

type SaveStatus = "idle" | "saving" | "saved" | "error";

export function WaypointEditorClient({ template, savedBlocks, clientSlug, saveEndpoint }: WaypointEditorClientProps) {
    ...
    const saveToServer = useCallback(async (document: any) => {
        ...
        try {
            ...
            const endpoint = saveEndpoint || `/api/templates/${template.id}`;
    ...
    useEffect(() => {
        ...
        if (template?.jsonContent && (template.jsonContent as any).root && Object.keys(template.jsonContent as any).length > 1) {
            const document = template.jsonContent as any;
            ...
            // If template is empty/newly created, we keep defaults from getConfiguration('') 
            // but we should at least initialize history
            const defaultDoc = editorStateStore.getState().document;
            initializeHistory(defaultDoc);
            lastSavedDocRef.current = JSON.stringify(defaultDoc);
            initializedTemplateId.current = template.id;
        }
        setInitialized(true);
    }, [template]);
